...
devin-2api translates Anthropic Messages / OpenAI Responses / Chat Completions requests into Connect-RPC `GetChatMessage` calls against the Devin upstream.
...
| Document             | Contents                                                                                                                                                                |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deployment.md`      | Platform layouts (launchd / systemd --user / Windows console), graceful drain, the one-writer state rule, rollback via a backed-up state copy, migration, offline smoke |
...
| `perf.md`            | Runtime diagnostics/profiling surface (`debug.pprof_listen`, `/debug/diagnostics/*`, per-platform capability matrix)                                                    |
...
| Document        | Contents                                                                                   |
| --------------- | ------------------------------------------------------------------------------------------ |
| `BENCHMARKS.md` | Measured same-host performance data vs the Go implementation — reference data, not a guide |

## Reading order for new operators

1. `README.md` quick start → running instance.
2. `deployment.md` → service management, migration, rollback.
